...
# ArgoCD version validation steps
...
def step_connect_to_eks_cluster(context, cluster_name, region):
    """Connect to EKS cluster for ArgoCD validation"""
...
def step_check_argocd_deployment(context):
    """Check ArgoCD controller deployment"""
    try:
        log_to_file(context.scenario_name, "Checking ArgoCD controller deployment...")
        
        # Get ArgoCD deployment
        cmd = "kubectl get sts argocd-application-controller -n argocd -o json"
        result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
        
        if result.returncode != 0:
            raise Exception(f"Failed to get ArgoCD deployment: {result.stderr}")
        
        deployment_data = json.loads(result.stdout)
        context.argocd_deployment = deployment_data
        ...
        log_to_file(context.scenario_name, f"Found ArgoCD deployment: {deployment_name}")
        ...
        argocd_container = None
        
        for container in containers:
            if container['name'] == 'application-controller':
                argocd_container = container
                break
        
        if not argocd_container:
            raise Exception("ArgoCD controller container not found in deployment")
        
        context.argocd_image = argocd_container['image']
        log_to_file(context.scenario_name, f"ArgoCD image: {context.argocd_image}")
        
    except Exception as e:
        log_to_file(context.scenario_name, f"Error checking ArgoCD deployment: {str(e)}")
        raise

@then('I should validate that ArgoCD version is "{expected_version}"')
def step_validate_argocd_version(context, expected_version):
    """Validate ArgoCD version"""
    try:
        log_to_file(context.scenario_name, f"Validating ArgoCD version is {expected_version}...")
        
        # Extract version from image tag
        image = context.argocd_image
        
        # Handle different image formats
        if ':' in image:
            # Format: public.ecr.aws/ArgoCD/ArgoCD:1.5.0
        ...
        if actual_version == expected_version:
            log_to_file(context.scenario_name, f"✓ ArgoCD version validation passed: {actual_version}")
            context.argocd_version_valid = True
        else:
            log_to_file(context.scenario_name, f"✗ ArgoCD version mismatch: expected {expected_version}, got {actual_version}")
            context.argocd_version_valid = False
            assert False, f"ArgoCD version mismatch: expected {expected_version}, got {actual_version}"
            
    except Exception as e:
        log_to_file(context.scenario_name, f"Error validating ArgoCD version: {str(e)}")
        context.ArgoCD_version_valid = False
        raise

@then('I should verify ArgoCD controller is running and healthy')
def step_verify_argocd_health(context):
    """Verify ArgoCD controller is running and healthy"""
    try:
        log_to_file(context.scenario_name, "Verifying ArgoCD controller health...")
        
        deployment = context.argocd_deployment
        ...
        if replicas > 0 and ready_replicas == replicas and available_replicas == replicas:
            log_to_file(context.scenario_name, "✓ ArgoCD controller is running and healthy")
            context.argocd_healthy = True
        else:
            log_to_file(context.scenario_name, "✗ ArgoCD controller is not fully healthy")
            context.argocd_healthy = False
            assert False, f"ArgoCD controller not healthy: {ready_replicas}/{replicas} replicas ready"
        
        # Additional health check - get pod status
        cmd = "kubectl get pods -n argocd -l app.kubernetes.io/name=argocd-application-controller -o json"
        result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
        ...
        if result.returncode == 0:
            ...
            if running_pods == len(pods) and running_pods > 0:
                log_to_file(context.scenario_name, "✓ All ArgoCD pods are running and ready")
            else:
                log_to_file(context.scenario_name, "⚠ Not all ArgoCD pods are running and ready")
        
    except Exception as e:
        log_to_file(context.scenario_name, f"Error verifying ArgoCD health: {str(e)}")
        context.argocd_healthy = False
        raise
